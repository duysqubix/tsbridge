...
func TestServiceWithWhoisMiddleware(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.whoisEnabled {
				whoisAdapter := &MockWhoisAdapter{whoisFunc: tt.whoisFunc}
...
func TestRegistry_RemoveService_VerifyStop(t *testing.T) {
	...
	// RemoveService calls server.Shutdown, which closes the listener. Poll the
	// listener address until it becomes unreachable to detect the stop.
	go func() {
		for {
